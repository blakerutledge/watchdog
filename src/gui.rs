...
impl RenderGroup {
    ...
    pub fn update<T>(
        ...
        draw_ui: &mut Box<dyn FnMut(&Context)>,
    ) {
        // EGUI + WINIT THINGZ
...
impl Gui {
    pub fn new() -> Gui {
        ...
        let render_group = RenderGroup::new(&window);
        ...
        let draw_ui = Box::new(|context: &Context| {
    ...
    pub fn update<T>(&mut self, event: winit::event::Event<'_, T>, control_flow: &mut ControlFlow) {
        *control_flow = ControlFlow::Poll;
        ...
        // Draw Egui Window
        self.render_group
            .update(&self.window, &event, &mut self.draw_ui);
